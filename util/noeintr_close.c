 ...
 * - if this is used in a multithreaded program, link to -lpthread as normal.
 * - if this is used in a single-threaded program, you should supply
 *   supply stubs for pthread_mutex_lock() and pthread_mutex_unlock().  These
 *   functions can be empty; the main point is to avoid having unresolved
 *   symbols which would be resolved with the real pthread library.
 */
